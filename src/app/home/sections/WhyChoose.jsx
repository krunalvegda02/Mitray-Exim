    ...
    <section className="relative py-12">
      <div className="relative">
        <div className="text-center mb-16 animate-reveal">
          <div className="inline-flex items-center px-4 py-1.5 bg-brand-gold/10 rounded-full mb-6 border border-brand-gold/20 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {reasons.map((reason, index) => (
            <div
              key={index}
              className="group relative glass-card p-10 rounded-[2.5rem] flex flex-col h-full animate-reveal"
              style={{ animationDelay: `${index * 100}ms` }}
            >
              {/* Number Background */}
              <span className="absolute top-6 right-8 text-7xl font-black text-slate-200/20 group-hover:text-brand-gold/5 transition-colors duration-500 pointer-events-none">
                {reason.number}
              </span>

              <div className="relative z-10 flex flex-col h-full">
                <div className={`w-14 h-14 bg-gradient-to-br ${reason.gradient} backdrop-blur-md rounded-2xl flex items-center justify-center text-white mb-8 shadow-xl group-hover:scale-110 transition-transform duration-500 border border-white/20`}>
                   <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                   </svg>
                </div>
                
                <h3 className="text-xl font-black mb-4 text-brand-navy tracking-tight group-hover:text-brand-gold transition-colors duration-300">
                  {reason.title}
                </h3>
                ...
                <div className="mt-auto pt-8">
                   <div className="w-12 h-1.5 bg-slate-100 group-hover:w-full group-hover:bg-brand-gold transition-all duration-700 rounded-full opacity-30"></div>
                </div>
              </div>
            </div>
          ))}
